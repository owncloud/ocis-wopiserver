# oCIS WOPI server: a proof of concept extension to open office files in ownCloud Infinite Scale

[![Rocket chat](https://img.shields.io/badge/Chat%20on%20Rocket.Chat-blue?logo=data:image/png;base64,)](https://talk.owncloud.com/channel/infinitescale)
[![Build Status](https://drone.owncloud.com/api/badges/owncloud/ocis-wopiserver/status.svg)](https://drone.owncloud.com/owncloud/ocis-wopiserver)
[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=owncloud_ocis-wopiserver&metric=security_rating)](https://sonarcloud.io/dashboard?id=owncloud_ocis)
[![Coverage](https://sonarcloud.io/api/project_badges/measure?project=owncloud_ocis-wopiserver&metric=coverage)](https://sonarcloud.io/dashboard?id=owncloud_ocis)
[![Go Report](https://goreportcard.com/badge/github.com/owncloud/ocis-wopiserver)](https://goreportcard.com/report/github.com/owncloud/ocis-wopiserver)
[![Go Doc](https://godoc.org/github.com/owncloud/ocis-wopiserver?status.svg)](http://godoc.org/github.com/owncloud/ocis-wopiserver)
[![oCIS docker image](https://img.shields.io/docker/v/owncloud/ocis-wopiserver?label=oCIS%20docker%20image&logo=docker&sort=semver)](https://hub.docker.com/r/owncloud/ocis-wopiserver)

## Install

You can download prebuilt binaries from the GitHub releases or from our [download mirrors](http://download.owncloud.com/ocis/wopi/). For instructions how to install this on your platform you should take a look at our [documentation](https://owncloud.github.io/extensions/ocis_wopi/)

## Development

Make sure you have a working Go environment, for further reference or a guide take a look at the [install instructions](http://golang.org/doc/install.html). This project requires Go >= v1.16. For the frontend it's also required to have [NodeJS](https://nodejs.org/en/download/package-manager/) and [Yarn](https://yarnpkg.com/lang/en/docs/install/) installed.

```console
git clone https://github.com/owncloud/ocis-wopiserver.git
cd ocis-wopiserver

yarn install
yarn build

make generate build

./bin/wopiserver -h
```

## Security

If you find a security issue please contact security@owncloud.com first.

## Contributing

Fork -> Patch -> Push -> Pull Request

## License

Apache-2.0

## Copyright

```console
Copyright (c) 2021 ownCloud GmbH <https://owncloud.com>
```
